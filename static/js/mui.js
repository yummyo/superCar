...
function(a, b) {
  var c, d, e = function(e, f) {
      ...
      case a.EVENT_END:
          if (!f.isFinal)
              return;
          for (var i = g.target, j = i, k = !1; j && j !== document; j = j.parentNode)
              (j.disabled || j.classList && j.classList.contains("mui-disabled")) && (k = !0);
          if (k)
              return;
  ...
  var c = "mui-slider"
    ...
    , n = a.Slider = a.Scroll.extend({
              ...
              if (m) {
                  ...
                  if (n.length > 0)
                      for (var g = 0, h = n.length; h > g; g++)
                          n[g].classList[g === d.slideNumber ? "add" : "remove"](j);
                  else {
                      ...
                      if (o)
                          ...
                          for (var p = m.querySelectorAll(".mui-control-item"), g = 0, h = p.length; h > g; g++)
                              p[g].classList[g === d.slideNumber ? "add" : "remove"](j)
                  }
              }
              b.stopPropagation()
          }
      ...
      _handleIndicatorTap: function(a) {
          var b = this
            , c = a.target;
          (c.classList.contains(g) || c.classList.contains(h)) && (b[c.classList.contains(g) ? "prevItem" : "nextItem"](),
          a.stopPropagation())
      },
...
function(a, b) {
  var c = "mui-active"
    ...
    , g = function(c, d) {
      ...
      "undefined" == typeof f.bottom && (f.bottom = c.offsetHeight + 1),
      this.itemElems = a.slice.call(c.querySelectorAll(e)),
      this.ids = [],
      this.shown = [],
  ...
  h.init = function() {
      this.itemElems && this.itemElems.length && (a.os.plus && "webview" === this.options.type ? a.plusReady(function() {
          this._initTabByWebview(),
          this.initEvent(),
          this.show(this.options.index)
      }
      .bind(this)) : (this._initTabByIframe(),
      this.initEvent(),
      this.show(this.options.index)))
  }
  ,
  h.initEvent = function() {
      this.element.addEventListener("tap", function(a) {
          for (var c = a.target; c && c !== b; c = c.parentNode)
              if (c.__mui_tab_id) {
  ...
  h._initTabByIframe = function() {
      for (var a = this.options.styles, c = parseFloat(a.top), d = parseFloat(a.bottom), e = parseFloat(window.innerHeight - c - d), g = 0, h = this.itemElems.length; h > g; g++) {
          var i = this.itemElems[g]
            , j = i.href
            , k = this._getId(i, j)
            , l = b.getElementById(k);
          l || (l = b.createElement("iframe"),
          l.id = k,
          l.src = j,
          l.setAttribute("style", f.replace("{DISPLAY}", "none").replace("{TOP}", c).replace("{BOTTOM}", d).replace("{HEIGHT}", e)),
  ...
  h.show = function(a, d) {
      ...
      if (~e) {
          ...
          if ("webview" === this.type)
              ~this.shown.indexOf(a) ? plus.webview.show(a) : (plus.webview.show(a, this.options.show.aniShow, this.options.show.duration),
              this.shown.push(a));
          else if ("iframe" === this.type) {
              var h = b.getElementById(a);
              h.style.display = "block"
          }
          -1 !== this.index && this.hide(this.ids[this.index]),
          this.index = e
      }
      return !1
  }
  ,
  h.hide = function(a) {
      if ("number" == typeof a && (a = this.ids[a]),
      "webview" === this.type)
          plus.webview.hide(a, "auto");
      else if ("iframe" === this.type) {
  ...
  a.fn.tab = function(a) {
      if (1 === this.length) {
          var b = this[0]
            , c = b.mui_plugin_tab;
          return a = a || {},
          c || (c = b.mui_plugin_tab = new g(b,a)),
          c
      }
  }
}(mui, document);
